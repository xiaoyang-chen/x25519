// copy from https://github.com/FiloSottile/age/blob/v1.0.0/x25519.go and update
...
import (
	...
	"github.com/xiaoyang-chen/x25519/internal/age/internal/bech32"
	// "filippo.io/age/internal/format"
	"github.com/xiaoyang-chen/x25519/internal/age/internal/format"

	githubPkgErrors "github.com/pkg/errors"
...
// add by xiaoyang-chen line start
...
func (i *X25519Identity) PrivateKey(prefix string) (private string, err error) {
	...
	if private, err = bech32.Encode(prefix, i.secretKey); err != nil {
		err = githubPkgErrors.Wrap(err, "bech32 encode fail")
		return
	}
	private = strings.ToUpper(private)
	return
}
...
func (i *X25519Identity) PublicKey(prefix string) (public string, err error) {
	...
	if public, err = bech32.Encode(prefix, i.ourPublicKey); err != nil {
		err = githubPkgErrors.Wrap(err, "bech32 encode fail")
	}
	return
}
...
func EncryptByX25519PublicKey(in []byte, publicKey, prefix string) (out []byte, err error) {
	...
	var hrp string
	var data []byte
	if hrp, data, err = bech32.Decode(publicKey); err != nil {
		err = githubPkgErrors.Wrap(err, "bech32 decode fail")
		return
	}
	if hrp != prefix {
		err = githubPkgErrors.Errorf("wrong prefix, correct: %s, wrong: %s", hrp, prefix)
		return
	}
	var r *X25519Recipient
	if r, err = newX25519RecipientFromPoint(data); err != nil {
		err = githubPkgErrors.Wrap(err, "new x25519 recipient from public key fail")
		return
	}
	var outBuff = bytes.NewBuffer(make([]byte, 0, 1024))
	var ioWC io.WriteCloser
	if ioWC, err = Encrypt(outBuff, r); err != nil {
		err = githubPkgErrors.Wrap(err, "get encrypt writer fail")
		return
	}
	if _, err = ioWC.Write(in); err != nil {
		err = githubPkgErrors.Wrap(err, "encrypt writer write fail")
		return
	}
	if err = ioWC.Close(); err != nil {
		err = githubPkgErrors.Wrap(err, "close encrypt writer fail")
		return
	}
	out = outBuff.Bytes()
	return
...
func DecryptByX25519PrivateKey(in []byte, privateKey, prefix string) (out []byte, err error) {
	...
	var hrp string
	var data []byte
	if hrp, data, err = bech32.Decode(privateKey); err != nil {
		err = githubPkgErrors.Wrap(err, "bech32 decode fail")
		return
	}
	if hrp != strings.ToUpper(prefix) {
		err = githubPkgErrors.Errorf("wrong prefix, correct: %s, wrong: %s", hrp, prefix)
		return
	}
	var i *X25519Identity
	if i, err = newX25519IdentityFromScalar(data); err != nil {
		err = githubPkgErrors.Wrap(err, "new x25519 identity from private key fail")
		return
	}
	var r io.Reader
	if r, err = Decrypt(bytes.NewReader(in), i); err != nil {
		err = githubPkgErrors.Wrap(err, "get decrypt reader fail")
		return
	}
	var outBuff = bytes.NewBuffer(make([]byte, 0, 1024))
	if _, err = io.Copy(outBuff, r); err != nil {
		err = githubPkgErrors.Wrap(err, "decrypt reader read fail")
		return
	}
	out = outBuff.Bytes()
	return
